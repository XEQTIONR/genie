...
import { create, update } from '@/routes/opportunities'
import {
  Field,
  FieldSet,
  FieldLegend,
...
export default function EditJobPosting({ projects, teams, job } : { projects: Project[], teams: Team[], job: Opportunity }) {
    ...
    useEffect(() => {
        ...
        const delta = quill.clipboard.convert({html: job.description})
    ...
    return (
        <AppLayout maxBodyWidth="md:max-w-7xl" breadcrumbs={breadcrumbs}>
            ...
            <form
                onSubmit={(e) => {
                    e.preventDefault()
                    put(update({opportunity: job}).url)
                }}
                onKeyDown={(e) => {
            ...
            >
                ...
                <Step step={1} heading={"General Information"}>
                    <FieldGroup className="mt-2 mb-10">
                        ...
                        <Field>
                            ...
                            <Combobox defaultValue={data.primary_role} onSelectValue={(val) => setData('primary_role', val)} placeholder="Select primary role" items={getRoleOptions()} />
                        </Field>
                ...
                <Step step={2} heading={"Job Options"}>
                    <Field className="w-full max-w-md mt-2">
                        ...
                        <Select defaultValue={data.owner_type?.toLowerCase() + '-' + data.owner_id} onValueChange={(value) => {
